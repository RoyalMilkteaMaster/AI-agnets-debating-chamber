...
T+2:30  每席必須留下第一份研究 checkpoint
T+3:30  尚未交出有效研究結果的席位啟動替補
T+4:45  停止接收新的研究結果，開始格式驗證與合併
T+5:00  產生不可變證據快照並進入投票與辯論
```

- 任何時間發生明確程序或 provider 錯誤，都立即重試，不等待 deadline。
- `T+3:30` 替補時，尚未用過重試的席位先使用相同模型；相同模型已重試失敗時改用其他可用模型。
- 同一席的原程序與替補都成功時，只採用第一份通過格式驗證的結果。
- 未採用的重複結果保存為診斷資料，不加入共享證據、辯論或投票。
- `T+4:45` 後完成的研究輸出不進入該次正式分析，避免越過 `T+5:00` 搜尋硬截止。
- 每席研究期間持續把證據卡寫入自己的目錄；`T+2:30` checkpoint 供同模型重試或替補接續，不直接產生額外票數。
...
```text
T+5:00       公布證據快照與初始立場，開始必跑的第一輪反方挑戰
T+6:30       第一輪最晚回覆；達 6 票立即結束
T+7:00       共識門檻切換為 5 票
T+6:30～8:30 未達門檻時進行第二輪；T+7 後達 5 票立即結束
T+8:30～9:45 仍未達 5 票時進行最後一輪
...
- Schema 驗證在研究期間增量執行，不等到 `T+4:45` 才開始。
- 無法在 `T+5:00` 前完成修復的內容不得加入正式證據快照。
...
- 至少完成一次七席同時執行的端到端壓力測試，七席均須在 `T+4:45` 前交出可驗證研究結果；只測單一 Agent 不算通過。
...
- 測試時可把正式分鐘壓縮成秒，驗證完整 `T+5／7／9／10／13` 狀態轉換。
- 必測：第一輪後 6 票、`T+7～9` 的 5 票、`T+10` 的 4 票、`3／2／2` 未達共識、Agent timeout／重試／替補、Format Repair、少於四票紅燈失敗、權限隔離，以及 Markdown／HTML 同源內容。
